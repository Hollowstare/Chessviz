...
bool checkMove(char coordinates[], char pole[][9])
{
    ...
    if (pole[beginY - 1][beginX] == 'p' || pole[beginY - 1][beginX] == 'P') {
        ...
        cout << "Incorrect entry, the type of piece(pawn) does not match the "
                "type of piece standing on the starting chessboard square"
             << endl;
        return false;
    }
    ...
    if (coordinates[2] == '-') {
        if (pole[endY - 1][endX] == ' ') {
            ...
            cout << "Incorrect entry, the figure cannot make a quiet move to "
                    "the occupied chessboard square"
                 << endl;
            return false;
        }
